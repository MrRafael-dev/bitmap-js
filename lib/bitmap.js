...
export class Surface extends Bitmap {
     ...
     * @returns {this}
     */
    blitsub(bitmap, x, y, cx, cy, width, height, mirrored = false, flipped = false, shaders = []) {
        // Percorrer pixels...
        for (let dy = 0; dy < height; dy += 1) {
            for (let dx = 0; dx < width; dx += 1) {
                const pixel = bitmap.getPixel(dx + cx, dy + cy);
                // Calcular posição do pixel...
                const px = mirrored ? (width - 1) - (x + dx) : x + dx;
                const py = flipped ? (height - 1) - (y + dy) : y + dy;
                // Desenhar pixel...
     ...
     * @returns {this}
     */
    blit(bitmap, x, y, mirrored = false, flipped = false, shaders = []) {
     ...
     * @returns {this}
     */
    text(bitmap, x, y, cx, cy, width, height, charset, charColumns, text, letterSpacing = 0, lineHeight = 0, mirrored = false, flipped = false, shaders = []) {
        // Posições do texto.
        let line = 0;
        let column = 0;
        ...
        for (let index = 0; index < text.length; index += 1) {
            ...
            // Desenhar caractere...
            this.blitsub(bitmap, charX, charY, charCutX, charCutY, width, height, mirrored, flipped, shaders);
