 ...
 */
var BitmapOffsets;
(function (BitmapOffsets) {
    /** Tamanho do cabeçalho (incluindo paleta). */
    BitmapOffsets[BitmapOffsets["HEADER_SIZE"] = 1078] = "HEADER_SIZE";
    /** Número mágico ("BM"). */
    BitmapOffsets[BitmapOffsets["HEADER_BM"] = 16973] = "HEADER_BM";
    /** (*Offset*) Número mágico ("BM"). */
    BitmapOffsets[BitmapOffsets["HEADER_MAGIC"] = 0] = "HEADER_MAGIC";
    /** (*Offset*) *Offset* dos ados da imagem. */
    BitmapOffsets[BitmapOffsets["HEADER_DATA"] = 10] = "HEADER_DATA";
    /** (*Offset*) Tamanho do arquivo, em *bytes*. */
    BitmapOffsets[BitmapOffsets["HEADER_FILESIZE"] = 2] = "HEADER_FILESIZE";
    /** (*Offset*) Largura do *bitmap*, em *pixels*. */
    BitmapOffsets[BitmapOffsets["HEADER_WIDTH"] = 18] = "HEADER_WIDTH";
    /** (*Offset*) Altura do *bitmap*, em *pixels*. */
    BitmapOffsets[BitmapOffsets["HEADER_HEIGHT"] = 22] = "HEADER_HEIGHT";
    /** (*Offset*) Formato de cores (*bits per pixel*). */
    BitmapOffsets[BitmapOffsets["HEADER_COLOR_FORMAT"] = 28] = "HEADER_COLOR_FORMAT";
    /** (*Offset*) Formato de compressão. */
    BitmapOffsets[BitmapOffsets["HEADER_COMPRESSION"] = 30] = "HEADER_COMPRESSION";
    /** Número de cores da paleta. */
    BitmapOffsets[BitmapOffsets["PALETTE_SIZE"] = 256] = "PALETTE_SIZE";
    /** (*Offset*) Posição da paleta de cores. */
    BitmapOffsets[BitmapOffsets["PALETTE_START"] = 54] = "PALETTE_START";
})(BitmapOffsets || (BitmapOffsets = {}));
/**
 * Cabeçalho padrão.
 *
...
export class Bitmap {
    ...
    static from(file) {
        ...
        /** (Número mágico ("BM"). */
        const signature = fileView.getUint16(BitmapOffsets.HEADER_MAGIC, false);
        /** *Offset* dos ados da imagem. */
        const dataOffset = fileView.getUint32(BitmapOffsets.HEADER_DATA, true);
        /** Largura do *bitmap*, em *pixels*. */
        const width = fileView.getUint32(BitmapOffsets.HEADER_WIDTH, true);
        /** Altura do *bitmap*, em *pixels*. */
        const height = fileView.getUint32(BitmapOffsets.HEADER_HEIGHT, true);
        /** Formato de cores (*bits per pixel*). */
        const bitsPerPixel = fileView.getUint16(BitmapOffsets.HEADER_COLOR_FORMAT, true);
        /** Formato de compressão. */
        const compression = fileView.getUint16(BitmapOffsets.HEADER_COMPRESSION, true);
        ...
        const fragment = file.slice(BitmapOffsets.PALETTE_START, file.byteLength);
    ...
    setPixel(x, y, primaryColor) {
        ...
        /** *Offset* do *pixel*. */
        const offset = BitmapOffsets.HEADER_SIZE + (this._width * iy) + px;
    ...
    getPixel(x, y) {
        ...
        /** *Offset* do *pixel*. */
        const offset = BitmapOffsets.HEADER_SIZE + (this._width * iy) + x;
    ...
    setColor(index, color) {
        ...
        /** *Offset* da paleta. */
        const offset = BitmapOffsets.PALETTE_START + (index * 4);
        // Escrever a nova cor...
