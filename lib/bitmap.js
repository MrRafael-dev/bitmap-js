...
export class Base64Encoder {
     ...
     * @returns {string}
     */
    getSegments(byte1, byte2, byte3, padding = 0) {
        /** Índice do primeiro caractere. */
        const charIndex1 = (((byte1 >>> 7) & 1) * 32)
            + (((byte1 >>> 6) & 1) * 16)
            + (((byte1 >>> 5) & 1) * 8)
            + (((byte1 >>> 4) & 1) * 4)
            + (((byte1 >>> 3) & 1) * 2)
        ...
        const charIndex4 = (((byte3 >>> 5) & 1) * 32)
            ...
            + ((byte3 >>> 0) & 1);
        /** Primeiro caractere. */
        const char1 = base64Table.charAt(charIndex1);
        /** Segundo caractere. */
        const char2 = base64Table.charAt(charIndex2);
        /** Terceiro caractere. (`=` quando `padding` é maior ou igual a 1) */
        const char3 = padding > 1 ? "=" : base64Table.charAt(charIndex3);
        /** Terceiro caractere. (`=` quando `padding` é maior ou igual a 2) */
        const char4 = padding > 0 ? "=" : base64Table.charAt(charIndex4);
    ...
    encode(data) {
        ...
        const size = Math.ceil(data.byteLength / 3) * 4;
        /** *Cache* para os segmentos de caracteres. */
        const segmentCache = [];
        /** *Cache* temporário. */
        const cache = new Uint8Array(3);
...
export class Base64Decoder {
     ...
     * @returns {Uint8Array}
     */
    restoreSegment(charIndex1, charIndex2, charIndex3, charIndex4) {
        /** Índice do primeiro *byte*. */
        const byte1 = (((charIndex1 >>> 5) & 1) * 128)
            + (((charIndex1 >>> 4) & 1) * 64)
            + (((charIndex1 >>> 3) & 1) * 32)
            + (((charIndex1 >>> 2) & 1) * 16)
            + (((charIndex1 >>> 1) & 1) * 8)
        ...
        const byte2 = (((charIndex2 >>> 3) & 1) * 128)
            + (((charIndex2 >>> 2) & 1) * 64)
            + (((charIndex2 >>> 1) & 1) * 32)
            + (((charIndex2 >>> 0) & 1) * 16)
            + (((charIndex3 >>> 5) & 1) * 8)
        ...
        const byte3 = (((charIndex3 >>> 1) & 1) * 128)
            + (((charIndex3 >>> 0) & 1) * 64)
            + (((charIndex4 >>> 5) & 1) * 32)
            + (((charIndex4 >>> 4) & 1) * 16)
            + (((charIndex4 >>> 3) & 1) * 8)
    ...
    decode(text) {
        ...
        let size = ((text.length / 4) * 3);
        /** Último caractere. */
        const lastChar = text.charAt(text.length - 1);
        /** Penúltimo caractere. */
        const penultChar = text.charAt(text.length - 2);
        ...
        for (let index = 0; index < text.length; index += 1) {
            ...
            const charIndex = base64Table.indexOf(char);
            // Encerrar operação quando alcançar um caractere de espaçamento:
            if (char === "=") {
                const charIndex1 = cache[0];
                const charIndex2 = cache[1];
                const charIndex3 = cache[2];
                const charIndex4 = cache[3];
                // Obter segmento (dividido)...
                const segment = this.restoreSegment(charIndex1, charIndex2, charIndex3, charIndex4).slice(0, cacheIndex - 1);
            ...
            if (cacheIndex === cache.byteLength) {
                ...
                const segment = this.restoreSegment(charIndex1, charIndex2, charIndex3, charIndex4);
...
export class Bitmap {
    ...
    setPixel(x, y, primaryColor) {
        ...
        /** *Offset* do *pixel*. */
        const offset = HEADER_SIZE + (this._width * iy) + px;
...
export class Surface extends Bitmap {
    ...
    pixel(x, y, primaryColor, shaders = []) {
        /** *Pixel* original. */
        let previous = new Pixel(x, y, this.getPixel(x, y));
        /** Novo *pixel*. */
