...
export class Bitmap {
    ...
    toImageData(mask = -1) {
        ...
        const data = new Uint8ClampedArray(this.pixels.byteLength * 4);
        // Percorrer pixels da imagem...
        for (let index = 0; index < this.pixels.byteLength; index += 1) {
     ...
     * @returns {this}
     */
    text(bitmap, x, y, cutX, cutY, width, height, columns, charset, text, spacingX = 0, spacingY = 0, mask = -1, flipX = false, flipY = false) {
        // Posições do texto.
        let line = 0;
        let column = 0;
        ...
        for (let index = 0; index < text.length; index += 1) {
            ...
            const charRow = Math.floor(charIndex / columns) % columns;
            console.log(`-> ${charRow} <-`);
            const charColumn = charIndex % columns;
            console.log(`charRow = ${charIndex} % ${columns} = ${charRow}`);
            console.log(`charColumn = ${charColumn} % (${charRow} * ${columns}) = ${charColumn}`);
            // Calcular valores de recorte...
            const charX = x + (column * width) + (spacingX * column);
            const charY = y + (line * height) + (spacingY * line);
            const charCutX = cutX + (charColumn * width);
            const charCutY = cutY + (charRow * height);
            console.log(`charX = (${x} * ${column}) + (${spacingX} * ${column}) = ${charX}`);
            console.log(`charY = (${y} * ${line}) + (${spacingY} * ${line}) = ${charY}`);
            console.log(`charCutX = ${cutX} + (${charColumn} * ${width}) = ${charCutX}`);
            console.log(`charCutY = ${cutY} + (${charRow} * ${height}) = ${charCutY}`);
            console.log(`----\n\n`);
            //console.log("Parameters");
            //console.log({ x, y, cutX, cutY, width, height, columns, charset, text, mask, flipX, flipY });
            //console.log("Calculations");
            //console.log({ charRow, charColumn, charX, charY, charCutX, charCutY });
            // Desenhar caractere...
            this.blitsub(bitmap, charX, charY, charCutX, charCutY, width, height, mask, flipX, flipY);
